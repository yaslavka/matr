...
class GameControls extends React.Component {
  ...
  betOne = () => {
    ...
    let totalValue = !this.state.winMsg
      ? this.state.total - Constants.betConstants.betOne
      : this.state.total + winAmount
  ...
  render() {
    return (
      <div className="sg-game-controls">
        <div className="sg-coins">Coins: {this.state.total}</div>
        ...
        <div className="sg-main-controls">
          <button
            disabled={this.state.gameOver || this.state.disableDoubleSubmit}
            className="sg-main-controls__bets"
            onClick={(e) => this.onUpdateBetMethod('betOne')}
          >
            Bet One
          </button>
          <button
            disabled={this.state.gameOver || this.state.disableDoubleSubmit}
            className="sg-main-controls__bets"
            onClick={(e) => this.onUpdateBetMethod('betMax')}
          >
            Bet Max
          </button>
          <button
            disabled={this.state.gameOver || this.state.disableDoubleSubmit}
            className="sg-main-controls__spin"
            onClick={this.onSpinReelsRepeat}
          >
            Spin
          </button>
        </div>
        <div
          onClick={(e) => this.setState({ winMsg: false, gameStarted: false })}
          className={this.state.winMsg ? 'winMsg' : 'hidden'}
        >
